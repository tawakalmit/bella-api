    ...
    <x-slot name="tableRow">
        @foreach ($newArrival as $item)
            <tr>
                <td>{{$loop->index + 1}}</td>
                <td>{{$item->image}}</td>
                <td><div class="w-[5rem]"><img class="w-full h-full object-cover" src="/storage/image/{{$item->image}}" alt="{{$item->image}}"></div></td>
                <td class="flex items-center justify-center flex-col">
                    <a class="flex justify-center items-center w-full" href="/new_arrival/{{$item->id}}/edit">Edit</a>
                    <form action="/new_arrival/{{$item->id}}/delete" method="post">
                        @csrf
                        @method('delete')
...
<div id="modal-add" class="invisible fixed flex items-center justify-center top-0 left-0 right-0 bottom-0 bg-[rgba(0,0,0,.5)]">
    <form action="/new_arrival" enctype="multipart/form-data" method="post" class="relative w-3/12 min-h-[25rem] rounded-xl bg-white p-10 flex flex-col gap-5 justify-between">
        @csrf
        ...
        <div class="hidden w-9/12"><input type="file" class="file-input w-full" id="imageInput" name="image" onchange="imageChange(this)" required/></div>
